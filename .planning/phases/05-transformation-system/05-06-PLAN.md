...
<task type="auto">
  <name>Task 1: Wire canvas edge interactions and transformation dialog integration</name>
  <files>
    src/app/mapper/components/MappingCanvas.tsx
    src/app/mapper/hooks/useMappingState.ts
    src/app/mapper/components/TransformationDialog.tsx
  ...
  <action>
    ...
    **page.tsx updates:**
    - Add a "Lookup Tables" button in the mapper page toolbar area (e.g., next to undo/redo buttons) that opens LookupTableManager as a modal
  </action>
  <verify>
  ...
  <done>Edge click opens transformation dialog. All transformation types visible in type selector. Transformed edges visually distinct with badge and color change. Dialog wired to store for save/remove.</done>
</task>

<task type="auto">
  <name>Task 2: Create lookup table manager and custom JS editor components</name>
  <files>
    src/app/mapper/components/LookupTableManager.tsx
    src/app/mapper/components/CustomJSEditor.tsx
    src/app/mapper/components/config-forms/LookupForm.tsx
  ...
  <verify>
    `npm run build` succeeds.
    Dev server running:
    1. Navigate to /mapper, open transformation dialog on an edge
    2. Select "Lookup Table" type - LookupForm renders with dropdown and "Manage Tables" button
    3. Click "Manage Tables" - LookupTableManager opens
    4. Create a new table, add entries, verify they persist on refresh
    5. Select "Custom JavaScript" type - CustomJSForm renders with code editor
    6. Enter `return input.toUpperCase();` and click "Test" - shows result
  </verify>
  <done>Lookup table management UI functional with CRUD. Custom JS editor with test capability. Both forms integrate into transformation dialog. All 7 transformation types fully configurable from the UI.</done>
</task>

<task type="checkpoint:manual-step" gate="blocking">
  <name>Task 3: Human verification of complete transformation system</name>
  <files>none</files>
  <action>
